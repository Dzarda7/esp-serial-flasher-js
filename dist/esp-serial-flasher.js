...
async function createEspSerialFlasherWasmModule(moduleArg = {}) {
  var moduleRtn;
...
function findWasmBinary() {
  ...
  if (Module['locateFile']) {
    return locateFile('esp-serial-flasher.wasm');
  }

  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL('esp-serial-flasher.wasm', import.meta.url).href;
...
var ASM_CONSTS = {
  167712: ($0) => { Module._timerEnd = Date.now() + $0; },  
 167752: () => { if (typeof Module._timerEnd === 'undefined') return 0; let r = Module._timerEnd - Date.now(); return r > 0 ? r : 0; }
};
function js_init_serial_buffer() { Module.serialTransportBuffer = Module.serialTransportBuffer || new Uint8Array(0); }
function __asyncjs__js_serial_write(data,size) { return Asyncify.handleAsync(async () => { if (!Module.serialTransport || typeof Module.serialTransport.write !== 'function') { console.error('[ERROR] Serial transport write() is not configured'); return -1; } try { let dataArray = new Uint8Array(size); for (let i = 0; i < size; i++) { dataArray[i] = HEAPU8[data + i]; } await Module.serialTransport.write(dataArray); return 0; } catch (error) { console.error('[ERROR] Serial write failed:', error); if (Module.logger && typeof Module.logger.error === 'function') { Module.logger.error('[ERROR] Serial write failed: ' + error.message); } return -1; } }); }
function __asyncjs__js_serial_read(data,size,timeout_ms) { return Asyncify.handleAsync(async () => { if (!Module.serialTransport || typeof Module.serialTransport.read !== 'function') { console.error('[ERROR] Serial transport read() is not configured'); return -1; } try { const bytes = await Module.serialTransport.read(size, timeout_ms); if (!bytes || bytes.length < size) { return -2; } for (let i = 0; i < size; i++) { HEAPU8[data + i] = bytes[i]; } return 0; } catch (error) { console.error('[ERROR] Serial read failed:', error); if (Module.logger && typeof Module.logger.error === 'function') { Module.logger.error('[ERROR] Serial read failed: ' + error.message); } return -1; } }); }
function __asyncjs__js_serial_enter_bootloader() { return Asyncify.handleAsync(async () => { if (!Module.serialTransport) { console.error('[ERROR] Serial transport is not configured'); return; } try { if (Module.logger && typeof Module.logger.debug === 'function') { Module.logger.debug('[JS] Entering bootloader mode'); } if (typeof Module.serialTransport.enterBootloader === 'function') { await Module.serialTransport.enterBootloader(); return; } if (typeof Module.serialTransport.setSignals !== 'function') { throw new Error('Serial transport setSignals() is not available'); } await Module.serialTransport.setSignals({ dataTerminalReady: false, requestToSend: false }); if (typeof Module.serialTransport.flush === 'function') { await Module.serialTransport.flush(); } await Module.serialTransport.setSignals({ dataTerminalReady: false, requestToSend: true }); await new Promise(resolve => setTimeout(resolve, 100)); await Module.serialTransport.setSignals({ dataTerminalReady: true, requestToSend: false }); await new Promise(resolve => setTimeout(resolve, 50)); await Module.serialTransport.setSignals({ dataTerminalReady: false, requestToSend: false }); await new Promise(resolve => setTimeout(resolve, 100)); if (typeof Module.serialTransport.flush === 'function') { await Module.serialTransport.flush(); } if (Module.logger && typeof Module.logger.debug === 'function') { Module.logger.debug('[JS] Bootloader entry sequence completed'); } } catch (error) { console.error('[ERROR] Failed to enter bootloader:', error); if (Module.logger && typeof Module.logger.error === 'function') { Module.logger.error('[ERROR] Failed to enter bootloader: ' + error.message); } } }); }
function __asyncjs__js_serial_reset_target(reset_hold_ms) { return Asyncify.handleAsync(async () => { if (!Module.serialTransport) { console.error('[ERROR] Serial transport is not configured'); return; } try { if (Module.logger && typeof Module.logger.debug === 'function') { Module.logger.debug('[PORT] Resetting target'); } if (typeof Module.serialTransport.resetTarget === 'function') { await Module.serialTransport.resetTarget(reset_hold_ms); return; } if (typeof Module.serialTransport.setSignals !== 'function') { throw new Error('Serial transport setSignals() is not available'); } await Module.serialTransport.setSignals({ dataTerminalReady: false, requestToSend: true }); await new Promise(resolve => setTimeout(resolve, reset_hold_ms)); await Module.serialTransport.setSignals({ dataTerminalReady: false, requestToSend: false }); } catch (error) { console.error('[ERROR] Failed to reset target:', error); if (Module.logger && typeof Module.logger.error === 'function') { Module.logger.error('[ERROR] Failed to reset target: ' + error.message); } } }); }
function __asyncjs__js_delay_ms(ms) { return Asyncify.handleAsync(async () => { await new Promise(resolve => setTimeout(resolve, ms)); }); }
function js_log_message(level,str) { const message = UTF8ToString(str); const logger = Module.logger; if (!logger) { return; } const method = level === 1 ? logger.error : level === 2 ? (logger.warn || logger.log) : level === 3 ? (logger.info || logger.log) : (logger.debug || logger.log); method?.call(logger, message); }
function __asyncjs__js_change_baud_rate(new_baud) { return Asyncify.handleAsync(async () => { try { if (!Module.serialTransport || typeof Module.serialTransport.setBaudRate !== 'function') { throw new Error('Serial transport setBaudRate() is not available'); } await Module.serialTransport.setBaudRate(new_baud); return 0; } catch (error) { console.error('[ERROR] Baud rate change failed:', error); if (Module.logger && typeof Module.logger.error === 'function') { Module.logger.error('[ERROR] Baud rate change failed: ' + error.message); } return 1; } }); }
...
var _flasher_disconnect = Module['_flasher_disconnect'] = makeInvalidEarlyAccess('_flasher_disconnect');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['flasher_disconnect'] != 'undefined', 'missing Wasm export: flasher_disconnect');
  ...
  _flasher_connect = Module['_flasher_connect'] = createExportWrapper('flasher_connect', 0);
  _flasher_change_baudrate = Module['_flasher_change_baudrate'] = createExportWrapper('flasher_change_baudrate', 1);
  _flasher_get_target = Module['_flasher_get_target'] = createExportWrapper('flasher_get_target', 0);
  _flasher_read_mac = Module['_flasher_read_mac'] = createExportWrapper('flasher_read_mac', 1);
  _flasher_get_security_info = Module['_flasher_get_security_info'] = createExportWrapper('flasher_get_security_info', 1);
  _flasher_flash_detect_size = Module['_flasher_flash_detect_size'] = createExportWrapper('flasher_flash_detect_size', 1);
  _flasher_flash_start = Module['_flasher_flash_start'] = createExportWrapper('flasher_flash_start', 3);
  _flasher_flash_write = Module['_flasher_flash_write'] = createExportWrapper('flasher_flash_write', 2);
  _flasher_flash_finish = Module['_flasher_flash_finish'] = createExportWrapper('flasher_flash_finish', 0);
  _flasher_flash_deflate_start = Module['_flasher_flash_deflate_start'] = createExportWrapper('flasher_flash_deflate_start', 4);
  _flasher_flash_deflate_write = Module['_flasher_flash_deflate_write'] = createExportWrapper('flasher_flash_deflate_write', 2);
  _flasher_flash_deflate_finish = Module['_flasher_flash_deflate_finish'] = createExportWrapper('flasher_flash_deflate_finish', 0);
  _flasher_flash_read = Module['_flasher_flash_read'] = createExportWrapper('flasher_flash_read', 3);
  _flasher_flash_erase = Module['_flasher_flash_erase'] = createExportWrapper('flasher_flash_erase', 0);
  _flasher_flash_erase_region = Module['_flasher_flash_erase_region'] = createExportWrapper('flasher_flash_erase_region', 2);
  _flasher_reset_target = Module['_flasher_reset_target'] = createExportWrapper('flasher_reset_target', 0);
  _flasher_disconnect = Module['_flasher_disconnect'] = createExportWrapper('flasher_disconnect', 0);
...
export default createEspSerialFlasherWasmModule;
